...
with constant of integration $$K$$. Note that the vector field is singular at $$v=-1$$ in accordance with the film height degenerating there. The corresponding first-order system is a Hamiltonian system given the Hamiltonian
...
Observe that the dynamics do $$\mathcal{H}(v_u,0)$$, as this is a saddle point and a homoclinic orbit only exists if $$\mathcal{H}(v_u,0)\leq \mathcal{H}(-1,0)$$.
...
We now analyse the bifurcation of $$\frac{2\pi}{k_0}$$-periodic evenn stationary solutions at Marangoni number $$M = M^* +4k_0^2 > M^*$$. We impose that the mass of the solution is conserved along a bifurcation curve, that is we impose
